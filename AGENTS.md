...
- **Agent A** must also read `AGENT_A.md` before implementation/research work.
- **Agent B** must also read `AGENT_B.md` before review/red-team/growth work.
- **Supervisor** must also read `.company/supervisor.md` and current `CMD:A:` / `CMD:B:` issues before issuing or changing commands.
