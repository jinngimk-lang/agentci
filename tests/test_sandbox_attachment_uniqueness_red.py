...
def test_pass_rejects_multiple_effective_attachments_for_same_workload_and_epoch():
    ...
    # Two simultaneously effective attachments for the same workload and policy
    # epoch make the actual enforced attachment ambiguous. A PASS event selecting
    # one attachment must not be allowed to turn that ambiguity into certainty.
    assert expected_verdict(document) != "PASS"
