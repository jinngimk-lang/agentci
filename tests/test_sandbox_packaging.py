...
def test_distribution_declares_lifecycle_attestation_data_files():
    pyproject = (ROOT / 'pyproject.toml').read_text(encoding='utf-8')
    assert 'share/agentci/sandbox/lifecycle-attestations' in pyproject
    assert 'examples/sandbox/lifecycle-attestations/*.json' in pyproject


def test_installed_verifier_redirects_lifecycle_attestation_lookup():
    verification = (ROOT / 'src' / 'agentci' / 'sandbox' / 'verification.py').read_text(encoding='utf-8')
    assert 'lifecycle_attestation' in verification
    assert "INSTALLED_ROOT / 'lifecycle-attestations'" in verification
