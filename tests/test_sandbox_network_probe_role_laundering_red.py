...
def test_network_security_assertion_cannot_launder_probe_obligation_through_unrelated_mandatory_assertion(monkeypatch):
    ...
    network_case = {
        ...
        # Malicious/ambiguous role metadata labels the actual security assertion
        # as utility while leaving an unrelated mandatory assertion outside the
        # utility set. The latter must not be allowed to carry the former's
        # canonical network-probe obligation.
    ...
    # The actual security assertion is supported only by a generic utility
    # event with no typed network channel. PR #62 currently exempts it because
    # some other mandatory non-utility assertion exists.
    ...
    # The unrelated mandatory assertion is changed to a nominal HTTPS network
    # event so it can satisfy the helper's guessed "dedicated probe" role.
