...
def test_explicit_probe_binding_to_unrelated_mandatory_assertion_cannot_create_pass(monkeypatch):
    ...
    network_case = {
        ...
        "probe": {
            ...
            # Deliberately bind the probe to the unrelated mandatory assertion,
            # not the assertion whose semantics claim the network boundary.
        ...
        # Mark the actual security assertion as utility so its generic utility
        # event can satisfy authorized-utility completeness if probe ownership
        # is trusted only by ID membership.
    ...
    # Explicit ownership metadata must not be accepted merely because the ID is
    # mandatory. The canonical contract must make the assertion/probe semantic
    # relationship machine-checkable, or fail closed when it cannot.
