...
def test_network_pass_cannot_bypass_channel_binding_via_denial_event_type(monkeypatch):
    ...
    # Convert the red control into a nominal PASS, but relabel its evidence as a
    # generic denial event. The canonical TestCase requires HTTPS, yet the
    # current channel matcher only checks event_type == "network" and therefore
    # skips channel binding for this semantically network-relevant evidence.
