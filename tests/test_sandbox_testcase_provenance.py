...
def test_pass_evidence_is_bound_to_immutable_testcase_content(tmp_path, monkeypatch):
    ...
    mutated_case["oracle"] = ["mutated oracle that changes certification semantics"]
    case_dir = tmp_path / "testcases"
    case_dir.mkdir()
    (case_dir / f"{fixture['case_id']}.json").write_text(json.dumps(mutated_case), encoding="utf-8")

    monkeypatch.setattr(sandbox_validator, "TEST_CASE_DIR", case_dir)
    sandbox_validator._load_test_case.cache_clear()
    try:
        assert sandbox_validator.expected_verdict(fixture) != "PASS"
    finally:
        sandbox_validator._load_test_case.cache_clear()
