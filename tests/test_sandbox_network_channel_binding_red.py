...
def test_network_pass_evidence_must_match_canonical_probe_channel(monkeypatch):
    ...
    network_case = {
        ...
        "threat_model": ["alternate-channel substitution"],
    ...
    # Turn the red-control assertion into a nominal PASS, but provide evidence
    # from UDP even though the canonical TestCase explicitly probes HTTPS.
    document["assertions"][0]["state"] = "PASS"
    document["verdict"] = "PASS"
    document["telemetry"][0]["layer"] = "network"
    primary_event = document["events"][0]
    primary_event["event_type"] = "network"
    primary_event["channel"] = "udp"
    primary_event["endpoint"] = "198.51.100.7:443"
    document["post_conditions"]["network_activity"] = "clean"
    _rebind_all(document)

    # Security invariant: one network channel cannot imply another. A UDP event
    # must not satisfy a canonical HTTPS assertion merely because it is a
    # healthy network event under the same policy attachment.
